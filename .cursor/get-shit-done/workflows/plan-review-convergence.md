...
```bash
INIT=$(node "/Users/patrykattc/work/git/kinder/.cursor/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "/Users/patrykattc/work/git/kinder/.cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "/Users/patrykattc/work/git/kinder/.cursor/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```
