...
TASKS_DIR="/Users/patrykattc/work/git/kinder/.cursor/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$( (find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null || true) | wc -l )
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/patrykattc/work/git/kinder/.cursor/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/patrykattc/work/git/kinder/.cursor/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
