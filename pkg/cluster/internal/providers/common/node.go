...
// Node is a shared nodes.Node implementation for container-based providers.
// BinaryName specifies the container runtime binary (e.g. "docker", "nerdctl", "podman").
type Node struct {
	Name       string
	BinaryName string
}
...
// nodeCmd implements exec.Cmd for container runtime nodes
type nodeCmd struct {
	binaryName string
...
func (c *nodeCmd) Run() error {
	...
	// args the command in the container rather than to the runtime binary
	args = append(
		args,
