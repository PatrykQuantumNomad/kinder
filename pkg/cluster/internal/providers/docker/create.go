...
// containers (e.g. constants.StrategyIPPinned or constants.StrategyCertRegen).
// An empty string means no strategy label is injected (single-CP path).
func planCreation(cfg *config.Cluster, networkName string, strategy string) (createContainerFuncs []func() error, err error) {
	...
	if haveLoadbalancer {
		...
		createContainerFuncs = append(createContainerFuncs, func() error {
			...
			return common.CreateContainer("docker", name, args)
		})
	}
	...
	for i, node := range cfg.Nodes {
		...
		case config.ControlPlaneRole:
			createContainerFuncs = append(createContainerFuncs, func() error {
				...
				// runArgsForNode) so the resulting `docker run` line is valid.
				...
				// Labels MUST be set at `docker run`; there is no container-update
				// equivalent (Docker API limitation). Strategy is decided once by
				// the Provision probe and captured in the closure.
				// Workers and LBs do NOT receive this label (no resume cost).
				if strategy != "" {
					// Insert --label before the image (the last element in args).
					// runArgsForNode always appends the image as the final arg.
...
func runArgsForNode(node *config.Node, clusterIPFamily config.ClusterIPFamily, name string, args []string) ([]string, error) {
	...
	args = append(args, common.GenerateMountBindings(node.ExtraMounts...)...)
	mappingArgs, err := common.GeneratePortMappings(clusterIPFamily, node.ExtraPortMappings...)
	if err != nil {
		return nil, err
	}
...
func runArgsForLoadBalancer(cfg *config.Cluster, name string, args []string) ([]string, error) {
	...
	mappingArgs, err := common.GeneratePortMappings(cfg.Networking.IPFamily,
		config.PortMapping{
	...
	// before the image per `docker run` arg order). Same label value as CPs/workers.
...
func createContainerWithWaitUntilSystemdReachesMultiUserSystem(name string, args []string) error {
	if err := common.CreateContainer("docker", name, args); err != nil {
		return err
	}
