...
<required_reading>
@/Users/patrykattc/work/git/kinder/.claude/get-shit-done/references/domain-probes.md
@/Users/patrykattc/work/git/kinder/.claude/get-shit-done/references/gate-prompts.md
@/Users/patrykattc/work/git/kinder/.claude/get-shit-done/references/universal-anti-patterns.md
</required_reading>
...
```bash
INIT=$(node "/Users/patrykattc/work/git/kinder/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(node "/Users/patrykattc/work/git/kinder/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-advisor 2>/dev/null)
```
...
- Read and execute @/Users/patrykattc/work/git/kinder/.claude/get-shit-done/workflows/discuss-phase-power.md end-to-end
...
```bash
TODO_MATCHES=$(node "/Users/patrykattc/work/git/kinder/.claude/get-shit-done/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "/Users/patrykattc/work/git/kinder/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @/Users/patrykattc/work/git/kinder/.claude/agents/gsd-advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "/Users/patrykattc/work/git/kinder/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
node "/Users/patrykattc/work/git/kinder/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "/Users/patrykattc/work/git/kinder/.claude/get-shit-done/bin/gsd-tools.cjs" state record-session \
...
```bash
node "/Users/patrykattc/work/git/kinder/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
**Full step-by-step instructions:** @/Users/patrykattc/work/git/kinder/.claude/get-shit-done/workflows/discuss-phase-power.md
