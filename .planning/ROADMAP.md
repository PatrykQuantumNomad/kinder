...
- SHIPPED **v2.4 Hardening** - Phases 52-58 + 57.1/57.2/57.3/57.4 (shipped 2026-05-31; released as v1.6)
...
<details>
<summary>SHIPPED v2.4 Hardening (Phases 52-58 + 57.1/57.2/57.3/57.4) - SHIPPED 2026-05-31 (released as v1.6)</summary>

See `.planning/milestones/v2.4-ROADMAP.md` for full phase details.

Phases 52-58 + inserted 57.1/57.2/57.3/57.4: HA Etcd Peer-TLS Fix (IP-pinning + 4-cert cert-regen fallback), Addon Version Audit & Bumps (local-path v0.0.36, Headlamp v0.42.0, cert-manager v1.20.2, Envoy Gateway v1.7.2; MetalLB/Metrics Server/registry:2 held), macOS Ad-Hoc Code Signing, Windows PR-CI Build Step, DEBT-04 Doctor Test Race Fix, Doctor Cosmetic Fixes (DIAG-05/06), plus inserted HA-resume hardening (Envoy LB cds/lds reapply, cluster-authoritative ip-family detection, HA cert-regen recovery across IPv4/IPv6/dual, IPAM probe `--privileged` fix), closed by live UAT closure for Phase 47 + 51. SYNC-05 (default node image bump to K8s 1.36.x) DEFERRED — `kindest/node:v1.36.x` not yet on Docker Hub. Released on the public version line as v1.6.
