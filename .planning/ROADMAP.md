...
- [x] **Phase 57.3: HA cluster cert-regen recovery (INSERTED 2026-05-16; SCOPE EXPANDED 2026-05-16)** - `RegenerateEtcdPeerCertsWholesale` widened from `etcd-peer` only to 4 cert types (etcd-peer + etcd-server + etcd-healthcheck-client + apiserver-etcd-client) with per-cert-type renew-then-cycle; active `etcdctl endpoint health` + apiserver `/healthz` polls replace 20s static `staticPodRecreationWait`; first-class `kinder resume --strategy=<auto|ip-pin|cert-regen>` cobra flag; IPv6/dual-stack loopback addressing via `loadbalancer.ClusterIPFamily` label; diagnostic dump on hard-fail; post-pass `kubeadm certs check-expiration` verification (completed 2026-05-17; verifier 4/4 PASSED). Live UAT on macOS Docker Desktop (IPv6-enabled `kind` network) closed SC1+SC2+SC3 across IPv4 (`uat-573-ipv4` 230s resume), IPv6 (`uat-573-ipv6` 502s via force-new-cluster bootstrap), and dual-stack (`uat-573-dual` 506s) HA fixtures; cert serials advanced for all 4 cert types in all 3 fixtures; apiserver `curl -k https://127.0.0.1:6443/healthz` and `https://[::1]:6443/healthz` both return 200 from inside cp1 post-resume. Plan 02 UAT cycle surfaced and fixed real downstream defects (force-new-cluster bootstrap for all-isolated etcd, rolling join for etcd3.6 quorum, WAL peer URL release-before-claim, dual-stack manifest IP patching, kubelet restart after manifest cycle) via 24 atomic `fix(57.3-02)` commits. SC4 closed by Plan 01 real-x509-verify + TLS-handshake regression test. SC5 (`hack/uat-47-ha-smoke.sh` test_09) remains dependent on Phase 57.4 landing — will close via Phase 58.
- [x] **Phase 57.4: IPAM probe regression — `inspect returned invalid IP` (INSERTED 2026-05-16)** - Root cause: Docker Desktop 4.73.0 does not populate `NetworkSettings.Networks` for non-privileged containers; `{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}` returns empty string; `net.ParseIP("")` returns nil; probe reports `inspect returned invalid IP "invalid IP"`. Fix: added `--privileged` to the IPAM probe `docker run` invocation (`ipamprobe.go:110`). Loud-log on ParseIP rejection added. ip-pin verdict announcement added at both docker and podman provider create paths. UAT: macOS Docker Desktop 4.73.0 — ALL 3 tests PASS (verdict=ip-pinned, ip-pin announcement at create, resume-strategy=ip-pinned label on cp1). nerdctl — EXPECTED (VerdictUnsupported). podman — INCONCLUSIVE (user env gcloud credential helper). Linux Docker — DEFERRED (no Linux host). (completed 2026-05-19; SUMMARY: 57.4-01-SUMMARY.md)
- [x] **Phase 58: Live UAT Closure for Phase 47 + 51** - Run and record live smoke tests against rebuilt v2.4 binary for both deferred UAT items (completed 2026-05-30; `hack/uat-47-ha-smoke.sh` + `hack/uat-51-envoy-ipvs-guide.sh` authored; live runs captured to `hack/uat-47-ha-smoke.log` + `hack/uat-51-envoy-ipvs-guide.log`; 47-UAT.md tests 3/9/12/13/14 flipped from issue→pass (passed: 14/14); 51-UAT.md augmented with v2.4 re-verification section — Option A preserves May 7 narrative; `make build` + 5-POSITIVE + 3-NEGATIVE strings-marker gate honored Pitfall 23 stale-binary risk; SC1-SC4 all green; v2.4 Hardening milestone is feature-complete pending milestone audit + ship)
...
**Plans**: 2 plans (`/gsd:plan-phase 57.3` 2026-05-16; executed 2026-05-17):
- [x] 57.3-01-PLAN.md — Diagnostic capture from uat-58-01; widen `RegenerateEtcdPeerCertsWholesale` cert scope to 4 cert types; active etcd + apiserver health-gates replace 20s static sleep; IPv6/dual-stack loopback addressing; post-pass `kubeadm certs check-expiration` verify; diagnostic dump on hard-fail; `--strategy=<auto|ip-pin|cert-regen>` cobra flag wired through ResumeOptions; FakeNode/FakeCmd unit-test matrix + real-x509-verify + TLS-handshake regression test (SC4); 6 atomic commits — SUMMARY: 57.3-01-SUMMARY.md
- [x] 57.3-02-PLAN.md — Live UAT `hack/uat-57.3-cert-regen.sh` covering IPv4 + IPv6 + dual-stack HA fixtures; 57.3-02-UAT.md evidence + verdict CLOSED (SC1+SC2+SC3 all PASS). 24 emergent `fix(57.3-02)` commits applied during UAT cycle to close real defects (force-new-cluster recovery, rolling join for etcd3.6, WAL peer URL release-before-claim, dual-stack manifest IP patching, kubelet restart) — SUMMARY: 57.3-02-SUMMARY.md
...
Plans:
- [x] 58-01-ha-smoke-PLAN.md — Phase 47 HA pause/resume live UAT against rebuilt v2.4 binary; flips 47-UAT.md tests 3/9/12/13/14 from issue to pass (completed 2026-05-30; 8/8 tests pass against HEAD e0ec855e; canonical log committed at hack/uat-47-ha-smoke.log; 47-UAT.md status diagnosed→closed)
- [x] 58-02-envoy-ipvs-guide-PLAN.md — Phase 51 Envoy LB + IPVS-1.36 reject + K8s 1.36 guide re-verification against rebuilt v2.4 binary; augments 51-UAT.md with v2.4 evidence section (completed 2026-05-30; 3/3 tests pass against HEAD 6b8c4f74; canonical log committed at hack/uat-51-envoy-ipvs-guide.log; 51-UAT.md Option A augment — May 7 narrative preserved; test_01 assertion relaxed via fix commit 6b8c4f74 for Docker docker.io/-stripping convention)
...
| 57. Doctor Cosmetic Fixes | v2.4 | 2/2 | Complete   | 2026-05-17 |
...
| 57.3. HA cluster cert-regen recovery (INSERTED; SCOPE EXPANDED) | v2.4 | 2/2 | Complete (cert-regen path live-UAT'd across IPv4 + IPv6 + dual; 24 emergent UAT fixes) | 2026-05-17 |
| 57.4. IPAM probe regression (INSERTED) | v2.4 | 1/1 | Complete (--privileged fix; macOS DD UAT ALL PASS) | 2026-05-19 |
| 58. Live UAT Closure for Phase 47 + 51 | v2.4 | 2/2 | Complete | 2026-05-30 |
