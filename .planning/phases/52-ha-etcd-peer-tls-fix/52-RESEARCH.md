...
| `pkg/cluster/internal/providers/podman/` | provider.go, provision.go, network.go | Podman network/container management | Mirrors docker path; `podman network connect --ip` supported |
...
All three open questions are resolved during planning. Resolutions are inline below; downstream work consumes the resolutions, not the questions.

1. **Probe image for scratch container** — RESOLVED.
   - Resolution: Use the `kindest/node` image referenced by `pkg/apis/config/defaults.Image`. It is always pulled by `kinder create cluster` so the probe inherits air-gap safety. Documented as the chosen image in Plan 52-01 Task 1 action (step f).

2. **`docker cp` write path for IPAM state at create time** — RESOLVED.
   - Resolution: Write at post-provision time, AFTER `errors.UntilErrorConcurrent(createContainerFuncs)` in `provider.Provision`. The container is running and `docker exec sh -c 'cat > /kind/ipam-state.json'` is the legal write path (Plan 52-02 Task 1 step iii). Pitfall 2 only applies to STOPPED-container reads; create-time write is on a running container.

3. **etcd peer cert SANs: do they include container IP or pod IP?** — RESOLVED (with live-verification deferred to UAT).
   - Resolution: kubeadm embeds the node's container IP (the kind-network-assigned IPv4) in `/etc/kubernetes/pki/etcd/peer.crt` SAN list. Pod-CIDR IPs are not in the SAN list because etcd peer traffic uses host network (the static pod runs in host network mode in kindest/node). This is the foundational assumption that makes both paths in this phase correct: the IP-pin path keeps the container IP stable so SANs continue to match; the cert-regen path regenerates SANs against the new container IP.
   - Risk if wrong: If kubeadm in some kindest/node version embedded pod-CIDR IPs instead, cert-regen would not fix peer TLS connectivity (it would regenerate against the same wrong IP, since pod-CIDR IPs are stable but irrelevant to the peer-TLS handshake source IP). Plan 52-01 cannot live-verify this because the probe runs on a scratch container with no kubeadm-installed cluster. Verification is therefore recorded as a UAT step in `52-VALIDATION.md` (Manual UAT-1) — to be executed against the first kinder-created HA cluster reached during execution. If UAT-1 reveals pod-CIDR IPs in the SAN list instead of container IPs, the cert-regen path's correctness assumption is invalidated and Plan 52-03 must be revised before merge.
   - Confidence: MEDIUM. Backed by kubeadm documentation and the existing kindest/node host-network static-pod pattern; not live-verified at plan time.
